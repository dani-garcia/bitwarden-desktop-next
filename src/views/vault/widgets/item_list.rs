...
pub fn view<'a>(
    ...
    let table_header = container(
        row![
            text("Name").size(13).color(colors.table_header),
            icons::ARROW_DOWN_UP.render(11.0, colors.table_header),
            Space::new().width(Fill),
            text("Options").size(13).color(colors.table_header),
        ]
        .spacing(4)
    ...
    let item_rows: Vec<Element<'a, ItemListMessage, AppTheme>> = items
        ...
        .map(|(i, item)| {
            ...
            let icon_circle = container(text(initial).size(13).color(colors.text_primary))
                .width(32)
                .height(32)
                .center_x(32)
                .center_y(32)
            ...
            container(
                buttons::ghost(content, is_selected, colors.item_hover, RADIUS_MD)
...
fn action_icon<'a>(icon: icons::Icon, message: ItemListMessage, colors: &AppColors) -> Element<'a, ItemListMessage, AppTheme> {
    buttons::ghost_icon(icon.render(14.0, colors.text_secondary))
        .on_press(message)
        .padding([4, 6])
        .into()
}
