...
**Decision**: The edit form's Save path encrypts via `CiphersClient::encrypt(CipherView)` and writes directly to the local `Repository<Cipher>`, skipping `CiphersClient::edit()` (which also PUTs to the server API).

**Rationale**: The app is a UI stub with no backend — the configured `api_url` points at an unused `localhost:8080`. Using `.edit()` would require a running mock server on every Save. The encrypt + repo.set path keeps the round-trip entirely local, matching how `list_ciphers` and `full_cipher` already work.
...
**Rationale**: The SDK has no `Repository<Organization>` available via the managed migrations, so these lists would otherwise have nowhere to live. Seeding from `mock.json` mirrors how the user profile itself is persisted (`mock.json` holds KDF + wrapped user key). Replaying the wrapped org key on unlock is what makes `CiphersClient::encrypt` / `decrypt` succeed for org-owned ciphers — without it, encryption fails with `MissingKeyId("Organization(...)")`.

## System-Tray Icon: `tray-icon` crate + shared `muda` receiver

**Decision**: Use the `tray-icon` crate (sister crate to `muda`) for the system tray. Tray menu events flow through `muda::MenuEvent::receiver()` — the same global receiver the native app menu already uses — so a single 16 ms poll drains both. Tray-icon click events (left-click toggle) come through `tray_icon::TrayIconEvent::receiver()` on the same poll tick.

**Version coupling**: `tray-icon` 0.22 pins `muda` 0.17. Our direct `muda` dep is held at `0.17` to match — attempting to mix `muda = "0.18"` (which we had briefly) splits the global event channel between the two versions and tray menu clicks are never seen by the app.

**Linux runtime requirement**: `tray-icon` on Linux needs `libayatana-appindicator3-1` (apt/.deb) or a `StatusNotifierItem` host in the DE (KDE ships one; GNOME needs the AppIndicator extension). When neither is present, `crate::tray::build()` returns `None` and the tray-gated settings become no-ops. Document this dep in the Linux installer scripts before shipping.
...
**Why not include a live file watcher**: the file is expected to be edited before launch as an early-days workaround, not during a run. A `notify` watcher is a small follow-up if needed; excluded to keep this change tight.
...
- **Windows**: named pipe at `\\.\pipe\bitwarden-desktop-next`. `tokio::net::windows::named_pipe::ServerOptions` on the primary (fresh pipe instance per accepted client); `std::fs::OpenOptions::open(...)` on the probe side (CreateFileW under the hood).

**Stale-socket self-healing (Unix)**: if the primary was hard-killed, the socket file lingers. On probe failure we know nobody is listening, so the file is definitionally stale and `cleanup_stale_socket` removes it before `bind`. Windows needs no cleanup — named pipes are kernel objects that vanish with the process.

**Listener lifetime**: bound inside an `iced::stream::channel`-backed `Subscription::run(instance_lock::wake_stream)`. The `fn` pointer is the subscription identity, so iced keeps the same stream instance across `update()` cycles instead of rebinding each tick.

**Why not the `single-instance` crate**: that crate only detects a duplicate — it doesn't deliver a wake signal to the primary, so a second launch would just exit silently and the user would think the app was broken. Writing it ourselves (~100 lines) delivers real UX and avoids pulling in dbus-rs (its Linux dep).
