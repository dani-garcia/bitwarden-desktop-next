...
fn main() {
    ...
    // Parse icon codepoints from CSS
    let css =
        fs::read_to_string(&css_file).unwrap_or_else(|e| panic!("Failed to read {css_file}: {e}"));
    ...
    fs::write(&gen_path, code).expect("Failed to write generated icons");
}
